...
static int my_verify_measurements(const char* mrenclave, const char* mrsigner,
    ...
    printf("I am in the verify measurements section. \n");
    assert(mrenclave && mrsigner && isv_prod_id && isv_svn);

    // Print mrenclave and g_expected_mrenclave in hex
    print_hex("mrenclave value", (const uint8_t*)mrenclave, 32);
    print_hex("g_expected_mrenclave value", (const uint8_t*)g_expected_mrenclave, 32);

    // Print mrsigner and g_expected_mrsigner in hex
    print_hex("mrsigner value", (const uint8_t*)mrsigner, 32);
    print_hex("g_expected_mrsigner value", (const uint8_t*)g_expected_mrsigner, 32);

    // Print isv_prod_id and g_expected_isv_prod_id in hex
    print_hex("isv_prod_id value", (const uint8_t*)isv_prod_id, 2);
    print_hex("g_expected_isv_prod_id value", (const uint8_t*)g_expected_isv_prod_id, 2);

    // Print isv_svn and g_expected_isv_svn in hex
    print_hex("isv_svn value", (const uint8_t*)isv_svn, 2);
    print_hex("g_expected_isv_svn value", (const uint8_t*)g_expected_isv_svn, 2);

    // Perform comparisons
    if (g_verify_mrenclave && memcmp(mrenclave, g_expected_mrenclave, 32)) {
        fprintf(stderr, "MRENCLAVE mismatch!\n");
        return -1;
    }
    if (g_verify_mrsigner && memcmp(mrsigner, g_expected_mrsigner, 32)) {
        fprintf(stderr, "MRSIGNER mismatch!\n");
        return -1;
    }
    if (g_verify_isv_prod_id &&
        memcmp(isv_prod_id, g_expected_isv_prod_id, 2)) {
        fprintf(stderr, "ISV_PROD_ID mismatch!\n");
        return -1;
    }
    if (g_verify_isv_svn && memcmp(isv_svn, g_expected_isv_svn, 2)) {
        fprintf(stderr, "ISV_SVN mismatch!\n");
        return -1;
    }
...
static int my_verify_callback(void* data, mbedtls_x509_crt* crt, int depth, uint32_t* flags) {
    ...
    printf("I am in the my verify callback. \n");
    if (depth != 0) {
        return MBEDTLS_ERR_X509_INVALID_FORMAT;
    }
    if (flags) {
        *flags = 0; // Ignore internal cert verification
    }

    // Extract quote details and print
    struct ra_tls_verify_callback_results* results = (struct ra_tls_verify_callback_results*)data;

    printf("Quote verification details:\n");
    printf("Attestation scheme: %d\n", results->attestation_scheme);
    printf("Quote verification result: 0x%x\n", results->dcap.quote_verification_result);

    printf("Quote (raw data):\n");
    for (size_t i = 0; i < crt->raw.len; i++) {
        printf("%02x", crt->raw.p[i]);
    }
    printf("\n");

    return ra_tls_verify_callback_extended_der_f(crt->raw.p, crt->raw.len, results);
}
...
void read_file(const char* file_path, char** buffer) {
    ...
    *buffer = (char*)malloc(file_size + 1);
    ...
    if (bytes_read != file_size) {
        fprintf(stderr, "Error: Could not read the full file %s (expected %ld bytes, got %zu bytes)\n", file_path, file_size, bytes_read);
        free(*buffer);
        fclose(file);
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char** argv) {
    ...
    mbedtls_printf("I am here (1). Basic init completed.\n");
    ...
    printf("(2) initialising mbedtls obejcts.. \n");
    ...
    mbedtls_printf("Can you see this?\n");
    printf("See this too? \n");
    ...
    mbedtls_printf("I am here...");
    fflush(stdout);
    mbedtls_printf("I am here2..");
    ...
    // message.mackeyshare_2 = "f0cf6099e629fd0bda2de3f9515ab72b"; //removed the hardcoding of the shares
