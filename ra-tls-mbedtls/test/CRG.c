...
int my_verify_callback(void* data, mbedtls_x509_crt* crt, int depth, uint32_t* flags) {
    ...
    return ra_tls_verify_callback(crt->raw.p, crt->raw.len,
...
int main(int argc, char** argv) {
    ...
    ssl_server_setup_and_handshake(ra_tls_create_key_and_crt_der_f, ra_tls_verify_callback,
                                   ra_tls_set_measurement_callback);
    // ssl_client_setup_and_handshake(ra_tls_create_key_and_crt_der_f, ra_tls_verify_callback,
    //                                ra_tls_set_measurement_callback);
}