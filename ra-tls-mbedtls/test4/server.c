...
// */ void (*ra_tls_set_measurement_callback_f)(int (*f_cb)(const char* mrenclave, const char*
// mrsigner,
...
int verify_player_details(char* kii_job_id, int recieved_other_player_number,
                          char* kii_job_id_defined, int other_player_number) {
    if (strcmp(kii_job_id, kii_job_id_defined) == 0 &&
        recieved_other_player_number == other_player_number) {
        return 0;
    } else {
        return -1;
    }
...
static int my_verify_callback(void* data, mbedtls_x509_crt* crt, int depth, uint32_t* flags) {
    ...
    return ra_tls_verify_callback_extended1_der_f(crt->raw.p, crt->raw.len,
...
int ssl_server_setup_and_handshake(
    ...
    ra_tls_verify_callback_extended_der_f ra_tls_verify_callback_extended1_der_f,
    ra_tls_set_measurement_callback_f ra_tls_set_measurement_callback1_f) {
    ra_tls_verify_callback_extended1_der_f = ra_tls_verify_callback_extended_der;
    ...
    // ra_tls_verify_callback_extended1_der_f                           = NULL;
    // ra_tls_set_measurement_callback1_f                               = NULL;
    ...
    (*ra_tls_set_measurement_callback1_f)(my_verify_measurements);
    ...
    //     char* error;
    //     ra_tls_create_key_and_crt_der_f = dlsym(ra_tls_attest_lib,
    //     "ra_tls_create_key_and_crt_der"); if ((error = dlerror()) != NULL) {
    //         mbedtls_printf("%s\n", error);
    //         return 1;
    //     }
    // } else {
